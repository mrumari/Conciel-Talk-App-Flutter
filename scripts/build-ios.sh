...
FLUFFYCHAT_ORIG_GROUP="im.concieltalk"
FLUFFYCHAT_ORIG_TEAM="4NXF6Z997G"
#FLUFFYCHAT_NEW_GROUP="com.example.concieltalk"
...
[ -n "${FLUFFYCHAT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${FLUFFYCHAT_ORIG_GROUP}.app/group.${FLUFFYCHAT_NEW_GROUP}.app/g" "ios/ConcielTalk Share/ConcielTalk Share.entitlements"
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for concieltalk.app and concieltalk.app.ConcielTalk-Share)
...
[ -n "${FLUFFYCHAT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/concieltalk.ipa"
  rm -rf "${TMPDIR}"
}
