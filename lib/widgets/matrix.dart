...
import 'package:concieltalk/utils/voip/uia_request_manager.dart';
...
class MatrixState extends State<Matrix> with WidgetsBindingObserver {
  ...
  // BackgroundNotifications? backgroundPush;

  Cube route = Cube.none;
  ...
  Client getLoginClient() {
    ...
    final candidate = _loginClientCandidate ??= ClientManager.createClient(
      '${AppConfig.applicationName}-${DateTime.now().millisecondsSinceEpoch}',
        ...
        switch (route) {
          case Cube.talk:
            widget.router!.currentState!.to('/talk'); // /rooms
            break;
          case Cube.book:
            break;
          case Cube.shop:
            break;
          case Cube.what:
            widget.router!.currentState!.to('/what');
            break;
          case Cube.where:
            widget.router!.currentState!.to('/where');
            break;
          case Cube.when:
            widget.router!.currentState!.to('/when');
            break;
          default:
        }
      });
    ...
    onLoginStateChanged[name] ??= c.onLoginStateChanged.stream.listen((state) {
      ...
      if (loggedInWithMultipleClients && state != LoginState.loggedIn) {
        ...
        if (state != LoginState.loggedIn) {
          widget.router!.currentState!.to('/biometrics'); // /rooms
        }
      } else {
        switch (route) {
          case Cube.talk:
...
/*
    if (PlatformInfos.isMobile) {
      backgroundPush = BackgroundNotifications(
        client,
        context,
        widget.router,
...
*/
  ...
  void initSettings() {
    ...
    store.getItem(SettingKeys.cubeRingScale).then(
          (value) => AppConfig.cubeRingScale =
              double.tryParse(value ?? '') ?? AppConfig.cubeRingScale,
        );
