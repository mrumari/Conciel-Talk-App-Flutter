...
class UpdateCheckerNoStore {
  ...
  Uri downloadUri(UpdateMetadata metadata) {
    // platform specific
    if (PlatformInfos.isWindows) {
      return Uri.parse('https://$gitLabHost/famedly/concieltalk/-'
          '/jobs/artifacts/$metadata/raw/'
          'build/windows/runner/Release/concieltalk.msix?job=build_windows');
    } else {
  ...
  Future<void> launchUpdater(UpdateMetadata metadata) async {
    ...
    try {
      ...
      if (PlatformInfos.isWindows) {
        ...
        if (response.statusCode == 200) {
          final file = File('${dir.path}/concieltalk.msix');
