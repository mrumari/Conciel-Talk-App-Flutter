...
class SettingsSecurityController extends State<SettingsSecurity> {
  void changePasswordAccountAction() async {
    final input = await showTextInputDialog(
      ...
      message: 'Enter existing password followed by new...',
      okLabel: L10n.of(context)!.ok,
      cancelLabel: L10n.of(context)!.cancel,
      textFields: [
        DialogTextField(
          hintText: 'Existing ${L10n.of(context)!.password}',
          obscureText: true,
          minLines: 1,
          maxLines: 1,
          autocorrect: false,
    ...
    final newLock = await showTextInputDialog(
      ...
      textFields: [
        DialogTextField(
          validator: (text) {
            if (text!.isEmpty ||
                (text.length == 6 && int.tryParse(text)! >= 0)) {
              return null;
            }
    ...
    final file = await showFutureLoadingDialog(
      ...
      future: () async {
        ...
        final exportFileName =
            'concieltalk-export-${DateFormat(DateFormat.YEAR_MONTH_DAY).format(DateTime.now())}.fluffybackup';
