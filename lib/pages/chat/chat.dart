...
import 'package:concieltalk/pages/chat/chat_view.dart';
import 'package:concieltalk/pages/chat/chat_send_actions.dart' as actions;
...
import 'package:concieltalk/utils/error_reporter.dart';
import 'package:concieltalk/utils/matrix_sdk_extensions/event_extension.dart';
import 'package:concieltalk/utils/matrix_sdk_extensions/ios_badge_client_extension.dart';
...
class ChatController extends State<ChatPageWithRoom> {
  ...
  void isEndDrawerOpen(bool isOpen) {
    setState(() {});
  }
  ...
  void recreateChat() async {
    ...
    final String share = VRouter.of(context).queryParameters['share'] ?? '';
    VRouter.of(context)
        .toSegments(['rooms', roomId], queryParameters: {'share': share});
  }
  ...
  void _loadFile() async {
    ...
    if (share == 'make-call') {
      onVoiceButtonTap();
    } else if (share != 'no-file') {
      final File sfile = File(share);
      final Uint8List fbytes = await sfile.readAsBytes();
      final MatrixFile mfile =
          MatrixFile.fromMimeType(bytes: fbytes, name: share);
      widget.room.sendFileEvent(mfile);
    }
  }
  ...
  void onAddPopupMenuButtonSelected(String choice) {
    if (choice == 'file') {
      actions.sendFileAction(context, room);
    }
    if (choice == 'image') {
      sendImageAction();
    }
  ...
  void onVoiceButtonTap() async {
    ...
    if (success.result != null) {
      final voipPlugin = Matrix.of(context).voipPlugin;
      try {
        await voipPlugin!.voip.inviteToCall(room.id, CallType.kVoice);
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(e.toLocalizedString(context))),
        );
  ...
  void onVideoButtonTap() async {
    ...
    if (success.result != null) {
      final voipPlugin = Matrix.of(context).voipPlugin;
      try {
        await voipPlugin!.voip.inviteToCall(room.id, CallType.kVideo);
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(e.toLocalizedString(context))),
        );
          ...
          child: Align(
            ...
            child: Transform.scale(
              ...
              child: Transform.translate(
                offset: Offset(195.r, 8.r),
                child: OuterDrawerRing(
                  context: context,
                  diameter: 288.r,
